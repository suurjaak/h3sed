...
class EquipmentPlugin(object):
    ...
    def render(self):
        ...
        if self._ctrls and all(self._ctrls.values()): # All built and still valid
            for prop in self.props():
                ...
                if choices_zh != ctrl.GetItems(): ctrl.SetItems(choices_zh)
                ctrl.Value = zh(value) if isinstance(value, str) else value
        ...
        try:
            for location, artifact in self._state.items():
                ...
                if not ctrl.Enabled:
                    ...
                    choices_zh = [zh(c) if isinstance(c, str) else c for c in choices]
                    if choices_zh != ctrl.GetItems(): ctrl.SetItems(choices_zh)
                    ctrl.Value = zh(artifact) if isinstance(artifact, str) else artifact or ""
                    ctrl.Enable()

                if not artifact and location in reserved_locations:
                    label = "<taken by %s>" % zh(self._state[reserved_locations[location]])
                    ctrl.SetItems([label])
                    ctrl.Value = label
                    ctrl.Disable()
    ...
    def on_change(self, prop, value, ctrl, rowindex=None):
        ...
        """
        value = en(value) if isinstance(value, str) else value
        v1, v2 = self._state[prop["name"]], value or None
        if v1 == v2: return False

        try: self._state[prop["name"]] = v2
        except Exception as e:
            v1_formatted = h3sed.hero.format_artifacts(v1 or "")
            ctrl.Value = zh(v1_formatted) if isinstance(v1_formatted, str) else v1_formatted
