...
"""Game major and minor version byte ranges for newer format, as (min, max)."""
NewVersionByteRanges = {
    "version_major":  (42, 42),
    "version_minor":  ( 0,  0),
}
...
def adapt(source, category, value):
    ...
    if not savefile or savefile.version != PROPS["name"]: return value
    is_new_format = savefile.match_byte_ranges(BytePositions, NewVersionByteRanges)
